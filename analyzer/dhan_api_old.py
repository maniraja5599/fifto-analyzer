...
class DhanHQIntegration:
    ...
    def __init__(self, client_id=None, access_token=None):
        ...
        self.symbol_map = {
            'NIFTY': {'security_id': '999920000', 'exchange': 'NSE'},
            'BANKNIFTY': {'security_id': '999920037', 'exchange': 'NSE'},
            'NIFTY50': {'security_id': '999920000', 'exchange': 'NSE'},
            'BANKEX': {'security_id': '999920037', 'exchange': 'NSE'}
        }
    ...
    def _get_fallback_price(self, instrument):
        ...
        price = fallback_prices.get(instrument.upper(), 25000.0)
        print(f"📈 Using fallback price for {instrument}: ₹{price:,.2f}")
        return price
    
    def test_connection(self):
        """Test DhanHQ API connection"""
        if not self.client or not self.client_id or not self.access_token:
            return False, "Client not initialized or credentials missing"
        
        try:
            print("🔄 Testing DhanHQ connection...")
            
            # Test with NIFTY quote
            quote_data = self.client.ticker_data(
                self.client_id,
                self.access_token,
                security_id='999920000',
                exchange_segment='NSE',
                instrument_type="INDEX"
            )
            
            print(f"📊 DhanHQ test response: {quote_data}")
...
# Global DhanHQ instance - Initialize with credentials from settings
try:
...
class DhanHQIntegration:
    ...
    def __init__(self, client_id=None, access_token=None):
        ...
        # Symbol mappings for DhanHQ
        self.symbol_map = {
            'NIFTY': {'security_id': '13', 'exchange': dhanhq.NSE, 'symbol': 'NIFTY 50'},
            'BANKNIFTY': {'security_id': '25', 'exchange': dhanhq.NSE, 'symbol': 'NIFTY BANK'},
            'NIFTY50': {'security_id': '13', 'exchange': dhanhq.NSE, 'symbol': 'NIFTY 50'},
            'BANKEX': {'security_id': '25', 'exchange': dhanhq.NSE, 'symbol': 'NIFTY BANK'}
        }
    ...
    def get_current_price(self, instrument):
        ...
        try:
            ...
            # Get live quote using ticker_data
            quote = self.dhan.ticker_data(
                symbol_data['security_id'],
                symbol_data['exchange'],
                dhanhq.INDEX
            )
            
            if quote and quote.get('status') == 'success':
                price = float(quote['data']['last_price'])
                print(f"✅ DhanHQ price for {instrument}: ₹{price}")
                return price
            else:
                print(f"⚠️  No price data from DhanHQ for {instrument}")
                return self._get_fallback_price(instrument)
    ...
    def get_historical_data(self, instrument, period='1y'):
        ...
        try:
            ...
            # Calculate date range (DhanHQ format: YYYY-MM-DD)
            end_date = datetime.now()
            if period == '1y':
                start_date = end_date - timedelta(days=365)
            elif period == '6m':
            ...
            # Get historical data using historical_daily_data
            historical_data = self.dhan.historical_daily_data(
                symbol_data['security_id'],
                symbol_data['exchange'],
                dhanhq.INDEX,
            ...
            if historical_data and historical_data.get('status') == 'success' and 'data' in historical_data:
                # Convert to pandas DataFrame
                df_data = historical_data['data']
                df = pd.DataFrame(df_data)
    ...
    def _format_option_chain(self, raw_data, instrument):
        """
        Format DhanHQ option chain to match NSE format
        """
        try:
            formatted_data = {
                'records': {
                    'underlyingValue': self.get_current_price(instrument),
            ...
            for option in raw_data['data']:
                ...
                option_data = {
                    'lastPrice': option.get('last_price', 0),
                ...
                if option_type == 'CE':
                    strikes[strike]['CE'] = option_data
                elif option_type == 'PE':
                    strikes[strike]['PE'] = option_data
            
            # Convert to list format
            formatted_data['records']['data'] = list(strikes.values())
            
            return formatted_data
            
        except Exception as e:
            print(f"❌ Error formatting option chain: {e}")
            return None
